...
var specTagMap = []specTagMapping{
	...
	{spec: "pilot-access-gateway.md", playbook: "pilot-access-gateway-apply.yml",
		exemptRows: map[string]string{
			...
			"AG37": "session_id never changing the HBAC/scope decision (it is only bound into a recorded session's ingest token) is proven by TestRunPortalOneShotConnect_Allowed/_Denied and TestConnectAuthorize_SessionIDOnlyBindsToken, not a playbook task",
			...
			"AG44": "recording precedence is the pure resolver's TestResolveRecordingMode, not a playbook task",
			"AG45": "unknown/invalid/reserved host policy denial is TestConnectAuthorize_RecordingPolicyDenies, not a playbook task",
			"AG46": "terminal mode without a session store is TestConnectAuthorize_TerminalWithoutStoreDenied, not a playbook task",
			"AG47": "metadata responses carrying no store coordinates or token is TestConnectAuthorize_MetadataCarriesNoIngestCredential, not a playbook task",
			"AG48": "ingest token claims binding and per-mint jti is TestConnectAuthorize_MintsBoundIngestToken, not a playbook task",
			"AG49": "fresh host policy on every authorize is TestConnectAuthorize_FreshPolicyEachRequest, not a playbook task",
			"AG50": "the interactive Portal taking the recorded path is TestConnectToHost_TerminalModeUsesRecordedPath, not a playbook task",
			"AG51": "no ssh before a successful session-store start is TestPortalTargetSession_StoreStartFailureNoSSH, not a playbook task",
			"AG52": "no local recording file is TestPortalTargetSession_NoLocalRecordingFile, not a playbook task",
