...
var specTagMap = []specTagMapping{
	...
	{spec: "pilot-access-gateway.md", playbook: "pilot-access-gateway-apply.yml",
		exemptRows: map[string]string{
			...
			"AG83": "recording precedence is the pure resolver's TestResolveRecordingMode, not a playbook task",
			"AG84": "unknown/invalid/reserved host policy denial is TestConnectAuthorize_RecordingPolicyDenies, not a playbook task",
			"AG85": "terminal mode without a session store is TestConnectAuthorize_TerminalWithoutStoreDenied, not a playbook task",
			"AG86": "metadata responses carrying no store coordinates or token is TestConnectAuthorize_MetadataCarriesNoIngestCredential, not a playbook task",
			"AG87": "ingest token claims binding and per-mint jti is TestConnectAuthorize_MintsBoundIngestToken, not a playbook task",
			"AG88": "fresh host policy on every authorize is TestConnectAuthorize_FreshPolicyEachRequest, not a playbook task",
			"AG89": "the interactive Portal taking the recorded path is TestConnectToHost_TerminalModeUsesRecordedPath, not a playbook task",
			"AG90": "no ssh before a successful session-store start is TestPortalTargetSession_StoreStartFailureNoSSH, not a playbook task",
			"AG91": "no local recording file is TestPortalTargetSession_NoLocalRecordingFile, not a playbook task",
			"AG92": "an idle healthy fail_closed session surviving is TestRecorderFailClosedIdleSessionSurvives, not a playbook task",
			"AG93": "the pre-connect recording notice is TestPortalTargetSession_RecordingNotice, not a playbook task",
			"AG95": "per-host userclass-unreadable denial (Phase 0 branch R) is TestConnectAuthorize_UserClassUnreadableDenies, not a playbook task",
			"AG96": "the recording show report is TestAccessRecordingShow_*, a read-only CLI, not a playbook task",
		}},
	{spec: "pilot-access-directory.md", playbook: "pilot-access-directory-apply.yml",
		exemptRows: map[string]string{
			...
			"AD31": "the Directory recording badge is TestDirectoryResolve_CarriesRecordingOverride and TestDirectoryHostLabel_RecordingBadge, not a playbook task",
		}},
	{spec: "pilot-session-store.md", playbook: "pilot-session-store-apply.yml",
		exemptRows: map[string]string{
			...
			"SS09": "master key file mode enforcement is loader-logic proven by internal/sessionstore's own Go test, not a playbook task",
			"SS10": "retention_days validation is a pre_tasks assert evaluated before any tagged task runs, not itself a tagged task",
			"SS11": "retention sweep purge-order is a multi-call sequence proven by unit tests, not a playbook task",
			"SS12": "replay gap detection is a multi-event-sequence property proven by internal/sessionstore's own Go test, not a playbook task",
			"SS13": "non-auditor-group denial needs a second peer identity, proven (partially — see docs/verification/pilot-session-store.md §3) by unit tests, not a playbook task",
			"SS14": "idempotent second-apply changed=0 is a multi-run property recorded in an evidence doc, not a single tagged task",
...
var tagCheckExemptSpecs = map[string]string{
	...
	"outbound-webhook.md":            "Go CLI + SQLite durable outbox + HTTP dispatcher feature (docs/tmp/now/spec.md), not a system-service playbook — verified by internal/outbound, internal/contract, internal/store, and cmd/pilot/cmd's own Go tests (see the spec's Checks probes), no apply playbook of its own",
}
