...
func pushDNSManifestPathPrompt(r *editRouterModel, dir string) tea.Cmd {
	...
	spec := tui.InputSpec{ScreenID: "dns.path", Title: "freeipa-dns manifest 檔路徑", Default: def}
...
func pushDNSManifestManager(r *editRouterModel, dir, path, banner string) tea.Cmd {
	...
	choices := []tui.Choice{
		{ID: "dns.manager.zones", Label: "🌐 Zones"},
		{ID: "dns.manager.preview", Label: "📋 顯示 normalized preview(套用後的最終 desired state)"},
		{ID: "dns.manager.back", Label: "↩  返回"},
	}
	title := fmt.Sprintf("管理 %s", path)
	spec := tui.SelectSpec{ScreenID: "dns.manager", Title: title, Choices: choices}
...
func pushDNSManifestCreateConfirm(r *editRouterModel, dir, path string) tea.Cmd {
	...
	spec := tui.ConfirmSpec{ScreenID: "dns.create_confirm", Title: question, Default: true}
...
func pushDNSManifestCreateDomainPrompt(r *editRouterModel, dir, path string) tea.Cmd {
	...
	spec := tui.InputSpec{ScreenID: "dns.create_domain", Title: label, Default: def, Validate: nonBlank}
	return r.transitionTo(r.uiFactory().Input(spec), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.InputScreen)
		if m.Canceled() {
			return pushTopMenu(r, dir, "")
...
func pushDNSManifestCreateRealmPrompt(r *editRouterModel, dir, path, domain string) tea.Cmd {
	...
	spec := tui.InputSpec{ScreenID: "dns.create_realm", Title: label, Default: strings.ToUpper(domain), Validate: nonBlank}
	return r.transitionTo(r.uiFactory().Input(spec), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.InputScreen)
		if m.Canceled() {
			return pushTopMenu(r, dir, "")
...
func pushDNSManifestCreateServerPrompt(r *editRouterModel, dir, path, domain, realm string) tea.Cmd {
	...
	spec := tui.InputSpec{ScreenID: "dns.create_server", Title: label, Default: def, Validate: nonBlank}
	return r.transitionTo(r.uiFactory().Input(spec), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.InputScreen)
		if m.Canceled() {
			return pushTopMenu(r, dir, "")
...
func pushDNSZonesMenu(r *editRouterModel, dir, path, banner string) tea.Cmd {
	...
	// A zone's own name is its stable identity everywhere else in the
	// manifest (records/preview/edit screens all reference it by name), so
	// it is also this row's Choice.ID — no synthetic per-row ID needed.
	choices := make([]tui.Choice, 0, len(names)+2)
	for _, n := range names {
		choices = append(choices, tui.Choice{ID: n, Label: "🌐 " + n})
	}
	choices = append(choices,
		tui.Choice{ID: "dns.zones.list.add", Label: "➕ 新增 zone"},
		tui.Choice{ID: "dns.zones.list.back", Label: "↩  返回"},
	)

	spec := tui.SelectSpec{ScreenID: "dns.zones.list", Title: fmt.Sprintf("Zones — %s", path), Choices: choices}
	return r.transitionTo(r.uiFactory().Select(spec), banner, func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.SelectScreen)
		if m.Canceled() {
...
func pushDNSZoneAddName(r *editRouterModel, dir, path string) tea.Cmd {
	...
	spec := tui.InputSpec{ScreenID: "dns.zone.add_name", Title: label, Validate: nonBlank}
	return r.transitionTo(r.uiFactory().Input(spec), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.InputScreen)
		if m.Canceled() {
			return pushDNSZonesMenu(r, dir, path, "")
...
func pushDNSZoneDetail(r *editRouterModel, dir, path, name, banner string) tea.Cmd {
	...
	// Fixed field menu: one namespaced ID per zone field, so a row's stable
	// identity survives its label (which embeds the live value).
	choices := []tui.Choice{
		{ID: "dns.zone.detail.name", Label: fmt.Sprintf("name：%s（唯讀，records 會用這個名稱互相參照）", name)},
		{ID: "dns.zone.detail.state", Label: fmt.Sprintf("state：%s", dnsStringOr(fields, "state", "present"))},
		{ID: "dns.zone.detail.records_mode", Label: fmt.Sprintf("records_mode：%s", dnsStringOr(fields, "records_mode", "(使用 manifest 預設)"))},
		{ID: "dns.zone.detail.acknowledge_split_horizon", Label: fmt.Sprintf("acknowledge_split_horizon：%s", dnsBoolDisplay(fields, "acknowledge_split_horizon"))},
		{ID: "dns.zone.detail.records", Label: fmt.Sprintf("Records（共 %d 筆）", len(records))},
		{ID: "dns.zone.detail.back", Label: "↩  返回"},
	}
	spec := tui.SelectSpec{ScreenID: "dns.zone.detail", Title: fmt.Sprintf("Zone %q — %s", name, path), Choices: choices}
	return r.transitionTo(r.uiFactory().Select(spec), banner, func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.SelectScreen)
		if m.Canceled() {
			return pushDNSZonesMenu(r, dir, path, "")
...
func pushDNSZoneStateField(r *editRouterModel, dir, path, name string) tea.Cmd {
	...
	spec := tui.SelectSpec{ScreenID: "dns.zone.field_state", Title: title, Choices: dnsEnumChoices("dns.zone.field_state", dnsZoneStateChoices)}
...
func pushDNSZoneRecordsModeField(r *editRouterModel, dir, path, name string) tea.Cmd {
	...
	spec := tui.SelectSpec{ScreenID: "dns.zone.field_records_mode", Title: title, Choices: dnsEnumChoices("dns.zone.field_records_mode", dnsRecordsModeChoices)}
	return r.transitionTo(r.uiFactory().Select(spec), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.SelectScreen)
		if m.Canceled() {
			return pushDNSZoneDetail(r, dir, path, name, "")
...
func pushDNSZoneSplitHorizonField(r *editRouterModel, dir, path, name string) tea.Cmd {
	...
	spec := tui.SelectSpec{ScreenID: "dns.zone.field_split_horizon", Title: title, Choices: dnsEnumChoices("dns.zone.field_split_horizon", dnsBoolChoices)}
	return r.transitionTo(r.uiFactory().Select(spec), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.SelectScreen)
		if m.Canceled() {
			return pushDNSZoneDetail(r, dir, path, name, "")
...
func pushDNSRecordsMenu(r *editRouterModel, dir, path, zoneName, banner string) tea.Cmd {
	...
	// A record's identity is the (name, type) pair — the exact composite key
	// DNSManifestRecord/SetDNSRecord address it by — so that pair, not the
	// row position, is this row's Choice.ID. The label stays the full
	// summary (which also embeds the mutable value/state).
	choices := make([]tui.Choice, 0, len(records)+2)
	for _, rec := range records {
		choices = append(choices, tui.Choice{
			ID:    dnsStringValue(rec, "name") + "/" + dnsStringOr(rec, "type", ""),
			Label: dnsRecordSummary(rec),
		})
	}
	choices = append(choices,
		tui.Choice{ID: "dns.records.list.add", Label: "➕ 新增 record"},
		tui.Choice{ID: "dns.records.list.back", Label: "↩  返回"},
	)

	spec := tui.SelectSpec{ScreenID: "dns.records.list", Title: fmt.Sprintf("Records — zone %q", zoneName), Choices: choices}
	return r.transitionTo(r.uiFactory().Select(spec), banner, func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.SelectScreen)
		if m.Canceled() {
...
func pushDNSRecordAddType(r *editRouterModel, dir, path, zoneName string) tea.Cmd {
	spec := tui.SelectSpec{ScreenID: "dns.record.add_type", Title: "新 record 的類型", Choices: dnsEnumChoices("dns.record.add_type", dnsRecordTypeChoices)}
	return r.transitionTo(r.uiFactory().Select(spec), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.SelectScreen)
		if m.Canceled() {
...
func pushDNSRecordAddName(r *editRouterModel, dir, path, zoneName, recordType string) tea.Cmd {
	...
	spec := tui.InputSpec{ScreenID: "dns.record.add_name", Title: label, Validate: nonBlank}
	return r.transitionTo(r.uiFactory().Input(spec), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.InputScreen)
		if m.Canceled() {
			return pushDNSRecordsMenu(r, dir, path, zoneName, "")
...
// dnsValueSourceChoicesFor is the one enum in this file offered by more than
// one screen (dns.record.add_value_source and dns.record.field_value_source),
// so — like rosterEnumChoices — its IDs are namespaced by the owning screen
// rather than shared, and use the manifest field each option writes
// (target.inventory_host / values) instead of the Chinese label text.
func dnsValueSourceChoicesFor(screenID string) []tui.Choice {
	return []tui.Choice{
		{ID: screenID + ".inventory_host", Label: dnsValueSourceChoices[0]},
		{ID: screenID + ".values", Label: dnsValueSourceChoices[1]},
	}
}

func pushDNSRecordAddValueSource(r *editRouterModel, dir, path, zoneName, recordType, name string) tea.Cmd {
	spec := tui.SelectSpec{ScreenID: "dns.record.add_value_source", Title: "值的來源", Choices: dnsValueSourceChoicesFor("dns.record.add_value_source")}
	return r.transitionTo(r.uiFactory().Select(spec), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.SelectScreen)
		if m.Canceled() {
			return pushDNSRecordsMenu(r, dir, path, zoneName, "")
...
func pushDNSRecordAddHostPicker(r *editRouterModel, dir, path, zoneName, recordType, name string) tea.Cmd {
	...
	spec := tui.SelectSpec{ScreenID: "dns.record.add_host_picker", Title: "要解析哪個 inventory host 的 ansible_host？", Choices: dnsHostChoices(names)}
	return r.transitionTo(r.uiFactory().Select(spec), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.SelectScreen)
		if m.Canceled() {
			return pushDNSRecordsMenu(r, dir, path, zoneName, "")
...
func pushDNSRecordAddValues(r *editRouterModel, dir, path, zoneName, recordType, name string) tea.Cmd {
	...
	spec := tui.InputSpec{ScreenID: "dns.record.add_values", Title: label, Validate: nonBlank}
	return r.transitionTo(r.uiFactory().Input(spec), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.InputScreen)
		if m.Canceled() {
			return pushDNSRecordsMenu(r, dir, path, zoneName, "")
...
func pushDNSRecordDetail(r *editRouterModel, dir, path, zoneName, recordName, recordType, banner string) tea.Cmd {
	...
	// Fixed field menu — see pushDNSZoneDetail. The value row's ID stays
	// "…detail.value" whichever of the two mutually exclusive value sources
	// (values / target.inventory_host) its label currently reflects.
	choices := []tui.Choice{
		{ID: "dns.record.detail.name", Label: fmt.Sprintf("name：%s（唯讀）", recordName)},
		{ID: "dns.record.detail.type", Label: fmt.Sprintf("type：%s（唯讀）", recordType)},
		{ID: "dns.record.detail.state", Label: fmt.Sprintf("state：%s", dnsStringOr(fields, "state", "present"))},
		{ID: "dns.record.detail.value", Label: fmt.Sprintf("%s：%s", valueLabel, valueDisplay)},
		{ID: "dns.record.detail.ttl", Label: fmt.Sprintf("ttl：%s", dnsIntDisplay(fields, "ttl"))},
		{ID: "dns.record.detail.back", Label: "↩  返回"},
	}
	title := fmt.Sprintf("Record %s %s — zone %q", recordName, recordType, zoneName)
	spec := tui.SelectSpec{ScreenID: "dns.record.detail", Title: title, Choices: choices}
	return r.transitionTo(r.uiFactory().Select(spec), banner, func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.SelectScreen)
		if m.Canceled() {
			return pushDNSRecordsMenu(r, dir, path, zoneName, "")
...
func pushDNSRecordStateField(r *editRouterModel, dir, path, zoneName, recordName, recordType string) tea.Cmd {
	...
	spec := tui.SelectSpec{ScreenID: "dns.record.field_state", Title: title, Choices: dnsEnumChoices("dns.record.field_state", dnsRecordStateChoices)}
	return r.transitionTo(r.uiFactory().Select(spec), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.SelectScreen)
		if m.Canceled() {
...
func pushDNSRecordValueSourceField(r *editRouterModel, dir, path, zoneName, recordName, recordType string) tea.Cmd {
	spec := tui.SelectSpec{ScreenID: "dns.record.field_value_source", Title: "值的來源", Choices: dnsValueSourceChoicesFor("dns.record.field_value_source")}
	return r.transitionTo(r.uiFactory().Select(spec), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.SelectScreen)
		if m.Canceled() {
			return pushDNSRecordDetail(r, dir, path, zoneName, recordName, recordType, "")
...
func pushDNSRecordValuesField(r *editRouterModel, dir, path, zoneName, recordName, recordType string) tea.Cmd {
	...
	spec := tui.InputSpec{ScreenID: "dns.record.field_values", Title: label, Default: current, Validate: nonBlank}
	return r.transitionTo(r.uiFactory().Input(spec), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.InputScreen)
		if m.Canceled() {
			return pushDNSRecordDetail(r, dir, path, zoneName, recordName, recordType, "")
...
func pushDNSRecordTargetField(r *editRouterModel, dir, path, zoneName, recordName, recordType string) tea.Cmd {
	...
	spec := tui.SelectSpec{ScreenID: "dns.record.field_target", Title: "要解析哪個 inventory host 的 ansible_host？", Choices: dnsHostChoices(names)}
	return r.transitionTo(r.uiFactory().Select(spec), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.SelectScreen)
		if m.Canceled() {
			return pushDNSRecordDetail(r, dir, path, zoneName, recordName, recordType, "")
...
func pushDNSRecordTTLField(r *editRouterModel, dir, path, zoneName, recordName, recordType string) tea.Cmd {
	...
	spec := tui.InputSpec{ScreenID: "dns.record.field_ttl", Title: label, Default: dnsIntValue(fields, "ttl"), Validate: dnsTTLValidator}
	return r.transitionTo(r.uiFactory().Input(spec), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(tui.InputScreen)
		if m.Canceled() {
			return pushDNSRecordDetail(r, dir, path, zoneName, recordName, recordType, "")
...
// dnsEnumChoices turns one of this file's fixed enum option slices
// (dnsZoneStateChoices, dnsRecordsModeChoices, dnsBoolChoices,
// dnsRecordTypeChoices, dnsRecordStateChoices) into tui.Choices whose stable
// IDs are the owning screen's ID plus the option's own value —
// "dns.zone.field_state.absent" and so on — so the several screens offering
// "present"/"absent" or "true"/"false" don't collide on a shared ID. The
...
// preserved. Mirrors edit_tui_roster.go's rosterEnumChoices.
func dnsEnumChoices(screenID string, values []string) []tui.Choice {
...
// dnsHostChoices renders hosts.yml entries for the two inventory_host picker
// screens. An inventory host's own name is its stable identity — it is
// exactly the string written into target.inventory_host — so it is also the
// row's Choice.ID.
func dnsHostChoices(names []string) []tui.Choice {
