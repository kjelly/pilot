...
func pushDNSManifestPathPrompt(r *editRouterModel, dir string) tea.Cmd {
	...
	return r.transitionTo(newTextInputModel("freeipa-dns manifest 檔路徑", def, nil), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(textInputModel)
		if m.Canceled() {
...
func pushDNSManifestCreateDomainPrompt(r *editRouterModel, dir, path string) tea.Cmd {
	...
	return r.transitionTo(newTextInputModel(label, def, nonBlank), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(textInputModel)
		if m.Canceled() {
			return pushTopMenu(r, dir, "")
		}
...
func pushDNSManifestCreateRealmPrompt(r *editRouterModel, dir, path, domain string) tea.Cmd {
	...
	return r.transitionTo(newTextInputModel(label, strings.ToUpper(domain), nonBlank), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(textInputModel)
		if m.Canceled() {
			return pushTopMenu(r, dir, "")
		}
...
func pushDNSManifestCreateServerPrompt(r *editRouterModel, dir, path, domain, realm string) tea.Cmd {
	...
	return r.transitionTo(newTextInputModel(label, "", nonBlank), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(textInputModel)
		if m.Canceled() {
			return pushTopMenu(r, dir, "")
		}
...
func pushDNSZonesMenu(r *editRouterModel, dir, path, banner string) tea.Cmd {
	...
	return r.transitionTo(newSelectModel(fmt.Sprintf("Zones — %s", path), items), banner, func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(selectModel)
		if m.Canceled() {
...
func pushDNSZoneDetail(r *editRouterModel, dir, path, name, banner string) tea.Cmd {
	...
	return r.transitionTo(newSelectModel(fmt.Sprintf("Zone %q — %s", name, path), items), banner, func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(selectModel)
		if m.Canceled() {
			return pushDNSZonesMenu(r, dir, path, "")
		}
...
func pushDNSZoneStateField(r *editRouterModel, dir, path, name string) tea.Cmd {
	...
	return r.transitionTo(newSelectModel(title, dnsZoneStateChoices), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(selectModel)
		if m.Canceled() {
...
func pushDNSZoneRecordsModeField(r *editRouterModel, dir, path, name string) tea.Cmd {
	...
	return r.transitionTo(newSelectModel(title, dnsRecordsModeChoices), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(selectModel)
		if m.Canceled() {
			return pushDNSZoneDetail(r, dir, path, name, "")
		}
...
func pushDNSZoneSplitHorizonField(r *editRouterModel, dir, path, name string) tea.Cmd {
	...
	return r.transitionTo(newSelectModel(title, dnsBoolChoices), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(selectModel)
		if m.Canceled() {
			return pushDNSZoneDetail(r, dir, path, name, "")
		}
...
func pushDNSRecordsMenu(r *editRouterModel, dir, path, zoneName, banner string) tea.Cmd {
	...
	return r.transitionTo(newSelectModel(fmt.Sprintf("Records — zone %q", zoneName), items), banner, func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(selectModel)
		if m.Canceled() {
...
func pushDNSRecordAddType(r *editRouterModel, dir, path, zoneName string) tea.Cmd {
	return r.transitionTo(newSelectModel("新 record 的類型", dnsRecordTypeChoices), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(selectModel)
		if m.Canceled() {
...
func pushDNSRecordAddValueSource(r *editRouterModel, dir, path, zoneName, recordType, name string) tea.Cmd {
	return r.transitionTo(newSelectModel("值的來源", dnsValueSourceChoices), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(selectModel)
		if m.Canceled() {
			return pushDNSRecordsMenu(r, dir, path, zoneName, "")
		}
...
func pushDNSRecordAddHostPicker(r *editRouterModel, dir, path, zoneName, recordType, name string) tea.Cmd {
	...
	return r.transitionTo(newSelectModel("要解析哪個 inventory host 的 ansible_host？", names), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(selectModel)
		if m.Canceled() {
			return pushDNSRecordsMenu(r, dir, path, zoneName, "")
		}
...
func pushDNSRecordStateField(r *editRouterModel, dir, path, zoneName, recordName, recordType string) tea.Cmd {
	...
	return r.transitionTo(newSelectModel(title, dnsRecordStateChoices), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(selectModel)
		if m.Canceled() {
...
func pushDNSRecordValuesField(r *editRouterModel, dir, path, zoneName, recordName, recordType string) tea.Cmd {
	...
	return r.transitionTo(newTextInputModel(label, current, nonBlank), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(textInputModel)
		if m.Canceled() {
			return pushDNSRecordDetail(r, dir, path, zoneName, recordName, recordType, "")
		}
...
func pushDNSRecordTTLField(r *editRouterModel, dir, path, zoneName, recordName, recordType string) tea.Cmd {
	...
	return r.transitionTo(newTextInputModel(label, dnsIntValue(fields, "ttl"), dnsTTLValidator), "", func(r *editRouterModel, s screen) tea.Cmd {
		m := s.(textInputModel)
		if m.Canceled() {
			return pushDNSRecordDetail(r, dir, path, zoneName, recordName, recordType, "")
		}
