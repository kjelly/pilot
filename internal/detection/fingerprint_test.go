...
func TestFingerprint_CategoryAndSeverityDoNotChangeFingerprint(t *testing.T) {
	...
	// episode in between.
	a := Fingerprint("web-1", SubjectKindManagedHost, "", "linux-host-v1", 1)
	b := Fingerprint("web-1", SubjectKindManagedHost, "", "linux-host-v1", 1)
	if a != b {
...
func TestFingerprint_ProfileVersionChangesFingerprint(t *testing.T) {
	v1 := Fingerprint("web-1", SubjectKindManagedHost, "", "linux-host-v1", 1)
	v2 := Fingerprint("web-1", SubjectKindManagedHost, "", "linux-host-v1", 2)
	if v1 == v2 {
