...
func TestFusion_ModelCanEscalateButNeverSuppress(t *testing.T) {
	local := LocalScoreResult{Valid: true, Score: 0.7, Category: "local-cat", Source: "baseline", Contributors: []Contributor{{Feature: "cpu", Score: 0.7}}}
	...
	if fr.Category != "model-cat" || fr.Source != "model" || fr.DetectorSource != "model" {
		t.Errorf("expected model category/source to win, got %+v", fr)
	}
	...
	if fr2.Category != local.Category || fr2.Source != "local" || fr2.DetectorSource != local.Source {
		t.Errorf("expected local category/source to win when model doesn't clear the margin, got %+v", fr2)
	}
}

// TestFusion_InsufficientDataUsesLocalScore (spec §19/§48).
func TestFusion_InsufficientDataUsesLocalScore(t *testing.T) {
	local := LocalScoreResult{Valid: true, Score: 0.8, Category: "local-cat", Source: "cohort", Contributors: []Contributor{{Feature: "mem", Score: 0.8}}}
	fr := FuseInsufficientData(local)
	if fr.Score != local.Score || fr.Category != local.Category || fr.Source != "local" || fr.DetectorSource != local.Source {
...
func TestFuseLocalOnly_MatchesLocalExactly(t *testing.T) {
	local := LocalScoreResult{Valid: true, Score: 0.42, Category: "c", Source: "log", Contributors: []Contributor{{Feature: "f", Score: 0.42}}}
