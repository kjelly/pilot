...
func (m MetricsSnapshot) WriteTextfile(path string) error {
	...
	// written by pilot-agent-controller. os.CreateTemp deliberately starts
	// at 0600; make the non-secret Prometheus exposition readable before the
	// atomic rename so the collector can ingest it.
