...
var roleContracts = []roleContract{
	...
	{Name: "prometheus", Description: "每站 Prometheus + Thanos Sidecar，自動從 host-monitoring/dcgm-exporter group 展開 scrape target (prometheus-apply.yml)", GroupVarsStem: "prometheus", VaultSections: []string{"thanos-s3", "node-exporter-auth", "dcgm-exporter-auth"}, HostVarsKeys: []string{"prometheus_site_label"}},
