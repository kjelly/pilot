...
| `playbooks/apply/*-apply.yml` | yes     | Hand-written apply playbook; peer-reviewed (lockout safety net lives here) |
| `playbooks/verify/*.yml`      | yes     | Generated by `pilot spec --generate` (auto-stamped) but **committed** so reviewers see what verify runs |
| `playbooks/generated/*.yml`   | **no**  | Legacy path; pre-spec-generator; ignore       |
...
| `inventory*.yaml`             | **no**  | Local exec artifact; regenerate via `pilot spec --to-inventory` |
