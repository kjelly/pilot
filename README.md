...
| **Ollama** | 0.5+ | 提供 LLM 推理 | https://ollama.com/download |
...
[3] Checking Ansible installation... OK (ansible [core 2.18.5])
[4] Checking 'ansible-lint' installation... OK
...
[8] Checking vm-target prerequisites (virt-customize + KVM + appliance DHCP)... OK
======================================
...
第一次跑約 30–90 秒（bleve BM25，無需 embedding），之後 ansible-core 沒改版會直接跳過：
...
| **RAG（檢索增強生成）** | pilot 會把 Ansible 模組文件切成 chunk，建成本地 bleve BM25 索引（無需 embedding），LLM 要查語法時就從本地撈。 |
...
依序檢查 Ollama 連線、LLM 模型、`ansible` / `ansible-playbook` 執行檔、SQLite、RAG 索引。出問題先跑這個。
...
任何工具呼叫都先變成 Proposal，你必須在 CLI 上按 `y/N`。`--auto-approve` 只是把這層**部分自動化**，並未移除。
...
- **首次跑約 30–90 秒**（bleve BM25，不需要 embedding 推論）。如果超過幾分鐘才奇怪。
