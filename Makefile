...
AGENT_CONTROLLER_VERSION ?= 0.0.0-dev
PILOT_ACCESS_GATEWAY_VERSION ?= 0.0.0-dev
...
pilot-cli-image: ## Build the deployment/control-node Docker image
	docker build \
		...
		--build-arg AGENT_CONTROLLER_VERSION=$(AGENT_CONTROLLER_VERSION) \
		--build-arg PILOT_ACCESS_GATEWAY_VERSION=$(PILOT_ACCESS_GATEWAY_VERSION) \
...
poc-checkmode-test: ## Full ephemeral fresh-host topology test of minimal-poc (L1 syntax, L3 check-mode dry-run, L4 apply, L5 verify, L6 idempotency). Requires VAULT=<path>; pass ROSTER=<path> too if PLAYBOOK touches freeipa-nfs-server. See AGENTS.md §1.4 and §4.0.
