...
| 27 | (v11.0 ground-up rebuild, 2026-07-21) The `pilot reconcile` wizard for `freeipa-identity` still **advertises** in its component-pick banner that it will ask "roster 檔路徑" next (`接下來會問你 roster 檔路徑`, with the same example filename `playbooks/apply/freeipa-identity.roster.example.yaml` it always shows), but the actual follow-up prompt chain — target_group → stage → --limit → --tags → vault `main.yaml` auto-detect → "vault needed?" → manual `extra -e` → preview → apply — never asks for the roster as a separate input. The roster **must** be supplied as the wizard's first-class `vars 檔路徑` (entered via the `需要 — 我有一份 ansible-vault 加密的 vars 檔` second-stage menu, after declining the auto-detected `.vault/main.yaml`), which produces the same end-state as a `@<file>` extra-var (the wizard's "vault" prompt compiles to `-e @<path>` internally — visible in the printed `▶ 套用：ansible-playbook playbooks/apply/freeipa-identity-apply.yml ... -e @...` line). v10.0's "zero extra `-e`" reading is therefore strictly literal — no chars typed in the manual `還有其他 -e 變數` field — but the roster **does** come in via the same `-e @<path>` mechanism v10.0 also used, just through the wizard's vault file slot rather than through the extra-vars slot. Confirmed live this round by walking the same prompt chain twice: first, typing `@/path/to/roster` into `還有其他 -e 變數` was rejected with `⚠ "@/path/to/roster" 不是 key=value 格式` (the field's `validateOptionalKV` rejects anything without a `=`); then the same path typed as the `vars 檔路徑` was accepted and the apply ran the roster correctly (`freeipa-server: ok=30 changed=11 failed=0`). | **Documentation gap, not a code bug**: the runbook's existing text in `freeipa-identity-apply.yml` already says "injected with `-e @<file>`" and the wizard's compiled command line shows the `@<file>` arg in the printed ansible-playbook line. The fix is a one-line clarification in the component-pick banner, replacing "接下來會問你 roster 檔路徑" with "接下來要你把 roster 檔當作 vault 檔案提供（偵測到的 main.yaml 拒絕後，從第二層選單選 `需要 — 我有一份 ansible-vault 加密的 vars 檔`，把路徑貼到 `vars 檔路徑` 欄）"; the rest of the pipeline is already correct. This is intentionally left as a one-line doc change, not a Go source change, because the v10.0 run completed cleanly with the same path (a previous agent just happened to know to use the vault slot) and the "right way" is fully captured by the printed `@<file>` arg. |
...
`tmp/pilot-verify-minimal-poc-r10/demo/{hosts.yml,inventory.yml,group_vars/,.vault/}`
