...
> （spec：`docs/runbooks/freeipa-ha-topology.yaml`）取代手動
...
的第三方視角）——由 `docs/runbooks/freeipa-ha-topology.yaml` 宣告式描述。
...
`vm-target down`（或 `vm-target topology down --spec docs/runbooks/freeipa-ha-topology.yaml`）
清掉再重來，避免 IP/狀態殘留。
...
`docs/runbooks/freeipa-ha-topology.yaml` 裡，內容摘要：
...
```bash
go run ./cmd/pilot vm-target topology up --spec docs/runbooks/freeipa-ha-topology.yaml
```
...
inventory : `pilot vm-target topology inventory --spec docs/runbooks/freeipa-ha-topology.yaml`
```
...
```bash
go run ./cmd/pilot vm-target list
go run ./cmd/pilot vm-target topology status --spec docs/runbooks/freeipa-ha-topology.yaml
```
...
| `ipa-replica-install` 失敗：`ERROR: Port check failed! Unable to resolve host name '<replica-fqdn>'` | primary 在 conncheck 時會反過來連回新 replica，沒有內建 DNS 時 primary 解析不到新節點 | 見 §2：`docs/runbooks/freeipa-ha-topology.yaml` 裡 `ipa-primary` 節點的 `wire:` 宣告，`topology up` 會自動把新 replica 的 FQDN/IP 冪等 pin 進 primary 的 `/etc/hosts`（不需要再手動下 `vm-target wire`） |
