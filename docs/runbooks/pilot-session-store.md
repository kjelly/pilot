...
|---|---|
| Fact timestamp | 2026-09-23T17:47Z |
| Target type | Disposable KVM vm-target topology (`phr-*`) |
| Inventory source | Grouped inventory generated by `pilot vm-target run --group freeipa-server=phr-ipa --group pilot-session-store=phr-store` |
| Actual targets | Session store `phr-store` (Ubuntu 24.04); gateway `phr-gw`; FreeIPA `phr-ipa` (`ipa-server-4.13.1-3.el9_8.2`) |
| Vault keys (names only) | `ipa_admin_password`, `pilot_session_store_master_key`, `pilot_session_store_ingest_signing_key`; for rollback only, the previous `pilot_session_store_ingest_token` |
| Previous revision used for rollback | `bd21971` (schema v1, static bearer token) |
...
[latest evidence record](../evidence/pilot-access-gateway/2026-09-24-per-host-session-recording.md).
...
| Field | Value |
|---|---|
| Verified at | 2026-09-23T17:47Z |
| Tested revision | `1de0964` (L22); the migration, backup and rollback code is unchanged through the final candidate `ad7c463` |
| Tested tree | `370d8cae780e3f5f6c718185ac915b263bb0fe32` (final candidate tree `88a9a5233718a936bb25de2773a31b6e6dcdfa3b`) |
| Target/inventory | `phr-store`, `phr-gw`, `phr-tb`, `phr-ipa` (generated vm-target inventories) |
| Upgrade | v1 DB with one recording → `index.db.pre-v1.bak` 0600 (schema v1), live DB schema v2 with `recording_policy_source`, `last_seq`, `ingest_jti`; the old recording listed and replayed |
| Rollback | backup restored; previous-revision store playbook `ok=41 changed=7 failed=0`; old binary opened the v1 DB and replayed the recording; no marker left on any host; previous-revision gateway: B authorized as `metadata` |
| Re-upgrade | current store, marker and gateway re-applied; B authorized as `terminal_output` / `host` |
| Evidence record | [Phase 8 evidence](../evidence/pilot-access-gateway/2026-09-24-per-host-session-recording.md) |
