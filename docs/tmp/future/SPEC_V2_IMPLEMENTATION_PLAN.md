...
| ComponentContract | **✅ catalog/lint；🟡 runtime wiring** | 22-component strict catalog、traceability/dependency/endpoint lint 與 deterministic preflight engine；deploy wiring 尚待 M0.4 |
...
**完成（2026-07-18，第一階段）**：`pilot spec migrate <v1.md>` 已建立保守的
v2 draft 與 `<out>.migration.json`。不明 action/traceability、v1 整數 matcher、
Jinja 與 applicability prose 都會產生機器可辨識的 `needsReview`/sidecar finding，
命令以非零退出；原始 v1 prose 以 quote context 留在草稿內，不會被格式轉換遺失。
既有正式 spec 的分批 target-test 遷移仍待實際 feature 修改時執行。
