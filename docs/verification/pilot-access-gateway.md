...
| AG37 | handoff | `session_id` 不影響 HBAC／scope 決策（spec.md §17.2：不把 session ID 當 proof）；只在 effective recording mode 為 terminal 時要求它是合法 UUID，並綁進 ingest token（per-host recording spec §17） | 0 | 見 `TestRunPortalOneShotConnect_Allowed`／`_Denied`（authorize 結果只取決於 target 是否在 scope 內）與 `internal/gatewayapi` 的 `TestConnectAuthorize_SessionIDOnlyBindsToken`（空、非 UUID、合法 UUID 三種 sid 對 metadata 連線結果相同、都不能讓 scope 外 target 變成 allow；terminal mode 下非 UUID 一律 deny `recording_session_id_invalid`） |
...
| AG44 | recording | resolver 依 per-host recording spec §4 precedence（host override > gateway default > built-in metadata）產生 mode 與 source | 0 | 見 `internal/gatewayapi` 的 `TestResolveRecordingMode`（table-driven，含 raw default `""` → `built_in_default`；unit test 覆蓋，非單一 shell 指令） |
| AG45 | recording | host policy unknown／invalid／保留值 `terminal_io` → Allowed=false 且帶對應 `deny_reason` | 0 | 見 `TestConnectAuthorize_RecordingPolicyDenies`（host_show 失敗、userclass 不可讀 → `recording_policy_unavailable`；重複 marker、保留值、zero-value policy → `recording_policy_invalid`） |
| AG46 | recording | effective mode 為 terminal 但沒有設定 session store → deny `recording_backend_unavailable`（沒有 local recording fallback） | 0 | 見 `TestConnectAuthorize_TerminalWithoutStoreDenied` |
| AG47 | recording | metadata 回應不含 store URL／CA／ingest token／recorder 參數 | 0 | 見 `TestConnectAuthorize_MetadataCarriesNoIngestCredential`（host 未設定與 `off` 兩種情形都比對原始 JSON） |
| AG48 | recording | terminal 回應的 ingest token claims 綁定 sid／user／gateway／scope／target／mode／source，lifetime = `max_session_duration`，且每次簽發的 `jti` 都不同 | 0 | 見 `TestConnectAuthorize_MintsBoundIngestToken` |
| AG49 | recording | 兩次 authorize 之間 host policy 由 absent 改為 `terminal_output`，第二次立即生效（gateway 沒有跨 request 的 policy cache） | 0 | 見 `TestConnectAuthorize_FreshPolicyEachRequest` |
| AG50 | recording | 互動式 Portal Connect 對 effective terminal mode 走 recorded path（與 Directory handoff 共用 `runPortalTargetSession`），不走 plain ssh | 0 | 見 `cmd/pilot/cmd` 的 `TestConnectToHost_TerminalModeUsesRecordedPath`（unit test 覆蓋，非單一 shell 指令） |
| AG51 | recording | recorded path 在 session store start 失敗時不啟動任何 ssh process（Phase A 也不做） | 0 | 見 `TestPortalTargetSession_StoreStartFailureNoSSH`；start 之後 Phase A／Phase B 失敗仍送 `complete=false` 的 finish，見 `TestPortalTargetSession_FailureAfterStartFinishesIncomplete` |
| AG52 | recording | terminal mode 不在 gateway 寫任何 local recording file（FileSink fallback 已移除） | 0 | 見 `TestPortalTargetSession_NoLocalRecordingFile` |
| AG53 | recording | `fail_closed` 下閒置但健康（沒有 pending event）的 session 超過 3×`failure_grace` 仍存活 | 0 | 見 `internal/sessionrecording` 的 `TestRecorderFailClosedIdleSessionSurvives`（6×grace） |
| AG54 | recording | terminal mode 在 SSH 前印出錄影提示（含 session id）；metadata 不印 | 0 | 見 `TestPortalTargetSession_RecordingNotice` |
...
| AG56 | recording | Phase 0 分支 R：gateway principal 對某台 host 的 `userclass` 沒有讀取權限 → 只有該 host 的連線 deny `recording_policy_unavailable`，同一 snapshot 的其他 host 不受影響 | 0 | 見 `internal/gatewayapi` 的 `TestConnectAuthorize_UserClassUnreadableDenies`；rights 解析見 `internal/freeipaaccess` 的 `TestParseHost_UserClassUnreadableIsUnavailable` |
| AG57 | recording | `sudo pilot access recording show <host>` 以 gateway 自己的 keytab 讀 host policy，依 §29 輸出 host policy、gateway default、effective、source（text／json）；非 root exit 2、host 不存在 exit 1 | 0 | 見 `cmd/pilot/cmd` 的 `TestAccessRecordingShow_*`；活體 L20 見 §4 |
