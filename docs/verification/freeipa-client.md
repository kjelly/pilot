...
| 2026-09-24 | v1.9 | Checklist 不變。實作面：(1) 全新 AlmaLinux 9 client 沒有 `dig`，DNS 預檢 rc=127 被 fail-closed gate 擋下——現在先檢查、缺少就安裝（EL `bind-utils`、Debian 經 apt framework 的 `bind9-dnsutils`），check-mode preview 對沒有 dig 的主機印訊息後 `end_host`；(2) EL 的 ipa-client 4.13.1 把 realm 區塊寫在 `/etc/krb5.conf.d/freeipa-realm`，failover reconcile 改為先找出實際檔案；(3) EL 上 `sssd-sudo.socket` 預設停用、socket 啟動的 sudo responder 連不到 backend，C8 在 EL 永遠 "not allowed"——RedHat 改為在 `services=` 保留顯式 `sudo` responder（NFS client overlay 寫同一行）；(4) 各 include 補上 `apply` tags、`ipa_cfg`/keytab stat 補 C11、auditd 改走 apt framework。4 台 vm-target topology（含一般 EL client）L3→L6 PASS，見 `docs/evidence/include-apply-tags-apt-framework/2026-09-24-dfb58a5.md` | pilot |
