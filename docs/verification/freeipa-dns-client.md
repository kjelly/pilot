...
| C3  | config   | `/etc/resolv.conf` 含 pilot 管理標記（證明由本 playbook 寫入，非 DHCP 預設）| ~pilot-freeipa-dns-client | grep pilot-freeipa-dns-client /etc/resolv.conf |
...
|----------|-------------------|
| C1 | `freeipa-dns-client — install dns query tool`／`tags: [C1]` |
| C2 | 隨 Debian/`systemd-resolved` 或 EL/`NetworkManager` 既有服務健康，本 playbook
  不重複起停該服務、只在其已 active 的前提下設定 DNS（見 pre_tasks gate）|
| C3/C4/C5 | Debian: `freeipa-dns-client — write systemd-resolved drop-in`
  + `freeipa-dns-client — write /etc/resolv.conf (symlink-safe)`／`tags: [C3, C4, C5]`；
  EL: `freeipa-dns-client — nmcli set DNS on active connection`／`tags: [C3, C4, C5]` |
