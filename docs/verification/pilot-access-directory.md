...
- **AD15 Gateway explicit authorization deny不得 retry繞過**：Phase 4 的 `connectToGateway` 目前無法從 ssh exit code分辨「明確 deny」與「session 正常結束」——這個區分需要 Phase 5 的 Gateway 端 one-shot connect 協定才存在，屆時要補一個能證明「deny 不會被當成 transport failure 而重試」的測試。目前只有「transport failure 會重試」這一半（AD14）已驗證。
...
- **AD20 Directory selection可 handoff到正確 scope Gateway**：`cmd/pilot/cmd.TestConnectToGatewayAllowed`（fake provider 驗證 argv/env 正確組成）；真正 SSH 到真實 Gateway 的活體驗證是 Phase 5 的責任（one-shot connect 協定的另一半在 Gateway 端還不存在）。
- **AD21 remote target `whoami` 為原登入 user / AD22 target exit 後回 Directory**：`cmd/pilot/cmd.TestConnectToGatewaySSHFailureReturnsToDirectory`/`TestConnectToGatewayCredentialFailureReturnsToDirectory` 證明「連線失敗一律回到 Directory 選單，不會讓整個 process 崩潰」；真人 SSH 到真實 target 驗證 `whoami` 是 Phase 5+vm-target 的工作。
...
- **AD25 session_id 每次 connect fresh unique / AD26 same session_id 出現在 Directory + Gateway metadata**：AD25 由 `internal/directoryapi.TestHandleConnectResolve_SessionIDIsFreshEveryCall` 涵蓋；AD26（session_id 貫穿 Gateway 端 metadata）要等 Phase 6 的 session correlation 才有東西可驗證。
