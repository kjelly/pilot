...
> 版本：v1.1（v1.0 的 13 條已在 pilot vm-target `almalinux-9` 上實跑 `ipa-server-install` + `pilot verify`；v1.1 新增 389-ds 目錄服務稽核日誌 C14–C16，已於 live `freeipa-server` vm-target 實跑 apply（`dsconf` 開啟稽核）+ `pilot verify` 得 16/16 PASS，見 §0 / §3 / §8）
...
- C1–C16 全部 `status=pass`（或 §5 允許的 `skip`）→ **PASS**：本機已可對外提供帳號 + sudo 管理，client 可 enroll，且目錄寫入操作留有稽核軌跡。
- 任一 `fail` → **FAIL**，常見修法：
...
| C16 | 稽核日誌的**輪替與磁碟用量**（`nsslapd-auditlog-maxlogsperdir` / `-logmaxdiskspace`）不在本 spec 範圍；預設值沿用 389-ds 出廠設定，長期落地站台請自行納入磁碟監控 | 全部 | 永久 |
