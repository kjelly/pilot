...
> 版本：v1.7（v1.0 的 13 條已在 pilot vm-target `almalinux-9` 上實跑 `ipa-server-install` + `pilot verify`；v1.1 新增稽核日誌 C14–C16；v1.2 將 DNS 與 NTP 服務改為可選啟用，並新增相容性檢測列 C17–C18；v1.3 修正 §0/§1/§1.5 對 `ipa_setup_dns`/`ipa_setup_ntp` 預設值的文件描述；v1.4 讓 C3/C12 驗證實際 configured FQDN，見 §0 / §3 / §8；v1.5 補上 kpasswd 464/tcp 的 C19，見 §8；v1.6 新增 C20，將 allow-recursion/allow-query-cache 從 FreeIPA 預設的 `trusted_clients` 開放給任意 client，見 §8；v1.7 修正 C16 的說明與排錯：389-ds audit log 有緩衝，apply 會等到檔案非空，見 §8）
...
| C16 | audit     | 稽核日誌檔存在且已寫入（非空；apply 會做一次目錄寫入，並等 389-ds 緩衝的紀錄寫進檔案） | 0                              | sudo test -s /var/log/dirsrv/slapd-IPA-PILOT-INTERNAL/audit |
...
- 任一 `fail` → **FAIL**，常見修法：
  ...
  - C16 fail → 稽核已開但檔案仍為空：389-ds 的 audit log 有緩衝（`nsslapd-auditlog-logbuffering: on`），寫入後要數秒到數十秒才會寫進檔案（2026-09-24 量到 8–29 秒），apply 已經在 dummy 寫入後等到檔案非空才結束；apply 結束後仍為空，表示 dummy 寫入沒成功或 dirsrv 寫不進檔案，可以手動做一次目錄寫入（如 `ipa user-mod admin --title=x`）再等半分鐘確認；
...
| C16 | 稽核日誌的**輪替與磁碟用量**（`nsslapd-auditlog-maxlogsperdir` / `-logmaxdiskspace`）不在本 spec 範圍；預設值沿用 389-ds 出廠設定，長期落地站台請自行納入磁碟監控 | C16 | 稽核已開但檔案仍為空：389-ds 的 audit log 有緩衝（`nsslapd-auditlog-logbuffering: on`），寫入後要數秒到數十秒才會寫進檔案（2026-09-24 量到 8–29 秒），apply 已經在 dummy 寫入後等到檔案非空才結束；apply 結束後仍為空，表示 dummy 寫入沒成功或 dirsrv 寫不進檔案，可以手動做一次目錄寫入（如 `ipa user-mod admin --title=x`）再等半分鐘確認；  |
...
| 2026-09-24 | v1.7 | C16 在沒有 client 納管的拓樸上 19/20 失敗。原因不是缺少寫入：`freeipa-server-apply.yml` 的 dummy 寫入成功了，但 389-ds 的 audit log 有緩衝（`nsslapd-auditlog-logbuffering: on`），紀錄 8–29 秒後才寫進檔案，verify 在 apply 結束 16 秒後就檢查。playbook 改為 dummy 寫入後用 `wait_for` 等檔案非空（最多 120 秒），再以 gate 斷言；不改 389-ds 的緩衝設定。Checklist 的指令與 Expected 不變，只改 C16 說明與兩處排錯文字。沒有 client 的 2 台拓樸上 20/20，見 `docs/evidence/freeipa-server/2026-09-24-266beca.md` | pilot |
