...
> 版本：v1.2（v1.0 的 13 條已在 pilot vm-target `almalinux-9` 上實跑 `ipa-server-install` + `pilot verify`；v1.1 新增稽核日誌 C14–C16；v1.2 將 DNS 與 NTP 服務改為可選啟用，並新增相容性檢測列 C17–C18，見 §0 / §3 / §8）
...
> 本檔是 FreeIPA **提供者**健康。時間同步預設依賴既有 `ntp` role（但可透過 `ipa_setup_ntp=true` 啟用由 FreeIPA 管理）；
> DNS 預設依賴既有 `dns`(unbound) role（但可透過 `ipa_setup_dns=true` 啟用由 FreeIPA 管理，見 §1.5）。
...
| DNS | **可選啟用內建 DNS**（預設關閉；由 `ipa_setup_dns=true` 啟用，啟用時自動安裝 `ipa-server-dns` 並設定 forwarders，否則使用 `--no-host-dns`）|
| NTP | **可選由 FreeIPA 管理 NTP**（預設關閉；由 `ipa_setup_ntp=true` 啟用並配置 chrony，否則使用 `--no-ntp`）|
...
| `ipa_setup_dns` | 是否啟用 FreeIPA 內建 DNS 服務，預設 `false` | 否（有預設）|
| `ipa_dns_forwarders` | 當啟用 DNS 時，上游 DNS 轉發器 IP 列表，預設 `[]`（無轉發器） | 否（有預設）|
| `ipa_setup_ntp` | 是否由 FreeIPA 管理/啟用 NTP 同步，預設 `false` | 否（有預設）|
