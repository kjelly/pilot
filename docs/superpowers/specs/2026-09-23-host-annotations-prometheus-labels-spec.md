...
| 2026-09-23 | v1.3 | 新增 `pilot edit` 頂層選單「Prometheus 主機註解 labels」+ structured actions `set_prometheus_annotation_label`/`delete_prometheus_annotation_label`（MCP 自動暴露）；Go 端驗證與 playbook gate 一對一，由 parity test 鎖住；`group_vars` 編輯器改為提示此 key（先前 block map 完全不顯示）；頂層選單改以 choice ID dispatch |
